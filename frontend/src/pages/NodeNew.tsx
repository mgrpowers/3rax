...
export default function NodeNew() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response = await nodeApi.create({ name, description });
      navigate(`/nodes`);
    } catch (error: any) {
  ...
  return (
    <div className="px-4 py-6 sm:px-0">
      ...
      <div className="bg-white rounded-lg shadow p-6 max-w-2xl">
        <h1 className="text-2xl font-bold text-gray-900 mb-6">Create New Node</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label htmlFor="nodeDescription" className="block text-sm font-medium text-gray-700 mb-2">
              Description
            </label>
            <textarea
              id="nodeDescription"
